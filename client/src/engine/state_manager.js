...
import EntityManager from "./entities/entity_manager.js";
...
export default class StateManager {
    ...
    /** @param {() => void} loop Function */
    set run_loop(loop) { this.runLoop = loop; }
    ...
    redrawUI = (from) => { 
        if (!this.updateUI)
            { console.error(`[StateManager]::redrawUI(${from})`); return; }
    ...
    set uv_scale(value) { this.state["uv_scale"] = value; this.redrawUI(`UV Scale(${JSON.stringify(value)})`); }
    ...
    set uv_offset(value) { this.state["uv_offset"] = value; this.redrawUI(`UV Offset(${JSON.stringify(value)})`); }
